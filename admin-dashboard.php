...
?>
    <div class="wrap">
        ...
        <div class="card" style="max-width: 100%; margin-top: 20px; padding: 10px 20px; background: #fff; border: 1px solid #ccd0d4; border-radius: 5px; box-shadow: 0 1px 3px rgba(0,0,0,.1);">
            <h2><?php _e('Test "Retain Original Image" Feature', 'webp-image-optimizer'); ?></h2>
            <p><?php _e('Click the button below to test if the "Retain Original Image" feature is working correctly:', 'webp-image-optimizer'); ?></p>
            <button type="button" id="test-retain-original" class="button button-secondary">
                <?php _e('Run Test', 'webp-image-optimizer'); ?>
            </button>
            <div id="test-results" style="margin-top: 10px; padding: 10px; background: #f8f8f8; border-left: 4px solid #46b450; display: none;"></div>
        </div>
    </div>

    <script type="text/javascript">
    jQuery(document).ready(function($) {
        $('#test-retain-original').on('click', function() {
            var $button = $(this);
            var $results = $('#test-results');
            
            $button.prop('disabled', true).text('<?php _e('Running Test...', 'webp-image-optimizer'); ?>');
            $results.hide();
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'test_retain_original',
                    nonce: '<?php echo wp_create_nonce('webp_test_retain_original'); ?>'
                },
                success: function(response) {
                    $button.prop('disabled', false).text('<?php _e('Run Test', 'webp-image-optimizer'); ?>');
                    
                    if (response.success) {
                        $results.html('<p>' + response.data.message + '</p>').show();
                    } else {
                        $results.html('<p>Error: ' + response.data.message + '</p>').show();
                    }
                },
                error: function() {
                    $button.prop('disabled', false).text('<?php _e('Run Test', 'webp-image-optimizer'); ?>');
                    $results.html('<p>Server error occurred. Please try again.</p>').show();
                }
            });
        });
    });
